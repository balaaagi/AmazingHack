...
public class GreaterAndEquals implements Operator {
	...
	public boolean check(Date conditionValue, Date itemFieldValue) {
		...
		else if(condition.get(Calendar.MONTH)>=item.get(Calendar.MONTH)){
			return true;
		}
		else if(condition.get(Calendar.DATE)>=item.get(Calendar.DATE)){
			return true;
		}
		else {
			return false;
